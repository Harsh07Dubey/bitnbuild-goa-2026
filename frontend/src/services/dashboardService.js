 ...
 */
export async function fetchMerchantDashboard(merchantId = 'MCH-DEMO01') {
  try {
    const res = await api.get('/merchant/dashboard', { params: { merchantId } });
    return res.data?.dashboard || res.data || MOCK_MERCHANT_DASHBOARD;
  } catch (err) {
    console.warn(`[dashboardService] fetchMerchantDashboard(${merchantId}) falling back to mock:`, err.message);
    return MOCK_MERCHANT_DASHBOARD;
  }
}

/**
 * Fetch Investor LP Dashboard metrics
 * GET /api/investor/dashboard
 */
export async function fetchInvestorDashboard(investorId = 'INV-DEMO01') {
  try {
    const res = await api.get('/investor/dashboard', { params: { investorId } });
    return res.data?.dashboard || res.data || MOCK_INVESTOR_DASHBOARD;
  } catch (err) {
    console.warn(`[dashboardService] fetchInvestorDashboard(${investorId}) falling back to mock:`, err.message);
